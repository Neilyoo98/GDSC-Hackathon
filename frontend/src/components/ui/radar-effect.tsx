...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-px w-full bg-gradient-to-r from-transparent via-[#39ff14] to-transparent" />
      </div>
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(232, 228, 220, ${0.28 - (idx + 1) * 0.025})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#e8e4dc33] bg-[#080808]">
        {icon || (
          <svg className="h-8 w-8 text-[#e8e4dc66]" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
          </svg>
        )}
      </div>
      <div className="hidden px-2 py-1 md:block">
        <div className="text-center text-xs font-bold text-[#e8e4dc99]">
          {text || "Agent Signal"}
        </div>
      </div>
    </motion.div>
